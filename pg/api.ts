...
*/
export function query<T>(config: ConnectionConfig, queryText: string, values: any[] = []): Promise<QueryResult<T> & {timeElapsed: number}> {
  return new Promise<QueryResult<T> & {timeElapsed: number}>((resolve, reject) => {
    ...
    pg.connect(config, (error, client, done) => {
      ...
      client.query(queryText, values, (error: Error, result: QueryResult<T>) => {
...
const api = {
  ...
  */
  databases(params: ConnectionConfig = {}) {
  ...
  */
  relations(params: ConnectionConfig & {database: string}): Promise<Relation[]> {
    const config = Object.assign({}, defaultClientConfig, params);
  ...
  */
  count(params: ConnectionConfig & {table: string}) {
    const config = Object.assign({}, defaultClientConfig, params);
  ...
  */
  query<T>(params: ConnectionConfig & {sql: string, variables: any[]}) {
    const config = Object.assign({}, defaultClientConfig, params);
